...
class Net(object):
    ...
    def modelvv(self,dropout,drug_layers,protein_strides,filters,fc_layers,prot_vec=False,prot_len=2500,activation='relu',
        ...
        model_d = Dense(target_dim, **params_dict)(model_d)
        model_d2 = Dense(target_dim, **params_dict)(model_d2)

        dense_d3 = Dense(target_dim, **params_dict)(input_d3)

        model_p = Dense(target_dim, **params_dict)(model_p)

        final_output = Dense(target_dim, **params_dict)(final_output)

        dense_p3 = Dense(target_dim, **params_dict)(input_p3)

        embeddings = [model_d, model_d2, dense_d3, model_p, final_output, dense_p3]
        # embeddings = Concatenate(axis=1)([model_d, model_d2, dense_d3, model_p, final_output, dense_p3])
        ...
        model_ttt = 0.5 * (square_of_sum - sum_of_square)
        model_ttt = Flatten()(model_ttt)            
