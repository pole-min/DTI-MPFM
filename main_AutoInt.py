...
from tensorflow.keras.layers import Input,Dense,BatchNormalization,Activation,Dropout,Embedding,SpatialDropout1D,Dot
...
from deepctr.models import AutoInt, NFM
import pandas as pd
class Net(object):
    ...
    def modelvv(self,dropout,drug_layers,protein_strides,filters,fc_layers,prot_vec=False,prot_len=2500,activation='relu',
        ...
        # Project different modalities to the same dimension
        projection_dim = 200  # Define the dimension for projection

        # drug_features = Concatenate()([model_d, model_d2, input_d3])
        # protein_features = Concatenate()([model_p, final_output, input_p3])

        model_d = Dense(projection_dim, **params_dict)(model_d)
        model_d2 = Dense(projection_dim, **params_dict)(model_d2)
        dense_d3 = Dense(projection_dim, **params_dict)(input_d3)
        model_p = Dense(projection_dim, **params_dict)(model_p)
        final_output = Dense(projection_dim, **params_dict)(final_output)
        dense_p3 = Dense(projection_dim, **params_dict)(input_p3)

        # Fusion using MultiHeadAttention and residual connections
        def multi_head_attention_with_residual(x, num_heads, key_dim):
            attention_output = MultiHeadAttention(num_heads=num_heads, model_size=projection_dim)(x,x,x,None)
            attention_output = Dropout(dropout)(attention_output)
            add_output = Add()([x, attention_output])
            return add_output

        model_ttt = multi_head_attention_with_residual(tensorflow.stack([model_d, model_d2, dense_d3, model_p, final_output, dense_p3], axis=1), num_heads=4,
                                                            key_dim=projection_dim)
        model_ttt = Concatenate(axis = 1)(tf.unstack(model_ttt, axis=1))
        # model_ttt = tf.reshape(model_ttt, (-1, projection_dim*6))
        ...
        # Step 10: Create final Keras model
        model_final = Model(inputs=[input_d, input_d2, input_d3, input_p, enc_inputs, input_p3, dec_inputs],
                            outputs=model_ttt)
        return model_final
