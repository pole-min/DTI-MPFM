...
class MultiHeadAttention(keras.Model):
    def  __init__(self, model_size, num_heads, **kwargs):
        ...
        self.WQ = keras.layers.Dense(model_size, name="dense_query",**params_dict)
        self.WK = keras.layers.Dense(model_size, name="dense_key",**params_dict)
        self.WV = keras.layers.Dense(model_size, name="dense_value",**params_dict)
        self.dense = keras.layers.Dense(model_size,**params_dict)
...
class FeedForwardNetwork(keras.Model):
    def __init__(self,dff_size,model_size):
        ...
        self.dense1 = keras.layers.Dense(dff_size,**params_dict)
        self.dense2 = keras.layers.Dense(model_size,**params_dict)
    def call(self,x):
        x = self.dense1(x)
        x = self.dense2(x)
        return x
...
class Encoder(keras.Model):
    def __init__(self,num_layers,model_size,num_heads,dff_size,vocab_size,maxlen,rate=0.1):
        ...
        self.embedding = keras.layers.Embedding(vocab_size,model_size,**embs_dict)
...
class Transformer(keras.Model):
    def __init__(self,num_layers,model_size,num_heads,dff_size,vocab_size,maxlen,traning=True,rate=0.1):
        ...
        self.final_dense = keras.layers.Dense(vocab_size,name='final_output',**params_dict)
